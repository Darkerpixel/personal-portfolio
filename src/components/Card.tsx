...
const updateCardPositions = (mx: number, my: number, cards: NodeListOf<Element>) => {
  cards.forEach((el) => {
    ...
    card.style.setProperty("--ratio-x", `${Math.min(Math.max(x / rect.width, 0), 1)}`);
    card.style.setProperty("--ratio-y", `${Math.min(Math.max(y / rect.height, 0), 1)}`);
    ...
    <>
      <div ref={containerRef} className="card-wrapper">
        {projects.map(({ frontmatter }, index) => (
          <div key={index} className="card-item" onClick={() => setOpenIndex(index)}>
        ...
        <MDXProvider components={MdxComponents}>
          <div className="modal-backdrop" onClick={() => setOpenIndex(null)}>
            <div className="modal-item" onClick={(e) => e.stopPropagation()}>
              <button className="modal-close-btn" onClick={() => setOpenIndex(null)}>
                ✕
              </button>
